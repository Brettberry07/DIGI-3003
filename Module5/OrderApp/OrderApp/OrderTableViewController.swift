    ...
    func configure(_ cell: UITableViewCell, forItemAt indexPath: IndexPath) {
        ...
        Task {
            if let image = try? await MenuController.shared.fetchImage(from: menuItem.imageURL) {
                // Update cell content configuration with the fetched image
                var updatedContent = cell.defaultContentConfiguration()
                updatedContent.text = menuItem.name
                updatedContent.secondaryText = menuItem.price.formatted(.currency(code: "usd"))
